...
# ________________________________________________________________________________
# --- FastAPI ROUTER ---
svg_rectangles_router = APIRouter(prefix="/v1/svg-parsing")


# ________________________________________________________________________________
# --- EXTRACT RECTANGLES DATA ---
@svg_rectangles_router.post(path="/extract-rectangles-data/",
                            responses={200: {}, 500: {}, 503: {}},
                            status_code=status.HTTP_200_OK,
                            response_class=UJSONResponse,
                            tags=["Extract RECTANGLES as JSON data"],
                            summary="Extract RECTANGLES data in JSON format.")
async def extract_rectangles_data(return_svg_size: bool = True,
    ...
    if not pdf_file.filename.endswith(".pdf"):
        raise HTTPException(status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE,
                            detail="ERROR -> '/extract-rectangles-data': File type not supported")

    # --- EXTRACT rectangles DATA ---
    rectangles_data, width, height, attributes = (SVGParser.
                                                  extract_svg_rectangles(pdf_file_obj=pdf_file,
                                                                         page_num=page_num,
                                                                         round_threshold=settings.ROUND_THRESHOLD)
                                                  )
    # \
    #     parallel_task_with_timeout(func_obj=SVGParser.extract_svg_rectangles,
    #                                pdf_file=pdf_file,
    #                                page_num=page_num)

    json_payload = {"rectangles_data": rectangles_data}
    ...
    # ________________________________________________________________________________
    # --- RETURN RESPONSE -> JSON WITH rectangles DATA ---
...
# ________________________________________________________________________________
# --- EXTRACT RECTANGLES DATA - S3 ---
@svg_rectangles_router.post(path="/extract-rectangles-data-s3/",
                            responses={200: {}, 500: {}, 503: {}},
                            status_code=status.HTTP_200_OK,
                            response_class=UJSONResponse,
                            tags=["Extract RECTANGLES as JSON data", "S3"],
                            summary="Extract RECTANGLES data in JSON format.")
async def extract_rectangles_data_s3(files_data: SVGParsingFilesDataS3) -> Response:
    ...
    # ________________________________________________________________________________
    rectangles_data, width, height, attributes = (SVGParser.
                                                  extract_svg_rectangles(pdf_file_obj=pdf_file_bytes,
