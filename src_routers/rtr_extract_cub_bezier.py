...
# ________________________________________________________________________________
# --- FastAPI ROUTER ---
svg_cub_bezier_lines_router = APIRouter(prefix="/v1/svg-parsing")


# ________________________________________________________________________________
# --- EXTRACT CUB BEZIER LINES DATA - S3 ---
@svg_cub_bezier_lines_router.post(path="/extract-cub-bezier-lines-data/",
                                  responses={200: {}, 500: {}, 503: {}},
                                  status_code=status.HTTP_200_OK,
                                  response_class=UJSONResponse,
                                  tags=["Extract CUBIC BEZIER LINES as JSON data"],
                                  summary="Extract CUBIC BEZIER LINES data in JSON format.")
async def extract_cub_bezier_lines_data(return_svg_size: bool = True,
    ...
    if not pdf_file.filename.endswith(".pdf"):
        raise HTTPException(status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE,
                            detail="ERROR -> '/extract-cub-bezier-lines-data': File type not supported")

    # --- EXTRACT cub_bezier DATA ---
    cub_bezier_data, width, height, attributes = SVGParser.extract_svg_cub_bezier_lines(pdf_file_obj=pdf_file,
                                                                                        page_num=page_num,
                                                                                        config=settings.SVG_ELEMENTS_EXTRACTION,
                                                                                        round_threshold=settings.ROUND_THRESHOLD)
    # \
    #     parallel_task_with_timeout(func_obj=SVGParser.extract_svg_cub_bezier_lines,
    #                                pdf_file=pdf_file,
    #                                page_num=page_num)

    json_payload = {"cub_bezier_lines_data": cub_bezier_data}
    ...
    # ________________________________________________________________________________
    # --- RETURN RESPONSE -> JSON WITH cub_bezier DATA ---
...
# ________________________________________________________________________________
# --- EXTRACT CUB BEZIER LINES DATA - S3 ---
@svg_cub_bezier_lines_router.post(path="/extract-cub-bezier-lines-data-s3/",
                                  responses={200: {}, 500: {}, 503: {}},
                                  status_code=status.HTTP_200_OK,
                                  response_class=UJSONResponse,
                                  tags=["Extract CUBIC BEZIER LINES as JSON data", "S3"],
                                  summary="Extract CUBIC BEZIER LINES data in JSON format.")
async def extract_cub_bezier_lines_data_s3(files_data: SVGParsingFilesDataS3) -> Response:
    ...
    # ________________________________________________________________________________
    # --- EXTRACT CUBIC BEZIER LINES DATA ---
    cub_bezier_data, width, height, attributes = (SVGParser.
                                                  extract_svg_cub_bezier_lines(pdf_file_obj=pdf_file_bytes,
