...
# ________________________________________________________________________________
# --- FastAPI ROUTER ---
svg_lines_router = APIRouter(prefix="/v1/svg-parsing")


# ________________________________________________________________________________
# --- EXTRACT LINES DATA ---
@svg_lines_router.post(path="/extract-lines-data/",
                       responses={200: {}, 500: {}, 503: {}},
                       status_code=status.HTTP_200_OK,
                       response_class=UJSONResponse,
                       tags=["SVG Parsing - Extract Lines as JSON data"],
                       summary="Extract LINES data in JSON format.")
async def extract_lines_data(return_svg_size: bool = True,
    ...
    if not pdf_file.filename.endswith(".pdf"):
        raise HTTPException(status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE,
                            detail="ERROR -> '/extract-lines-data': File type not supported")

    # --- EXTRACT LINES DATA ---
    lines_data, width, height, attributes = SVGParser.extract_svg_lines(pdf_file_obj=pdf_file,
                                                                        page_num=page_num,
                                                                        round_threshold=settings.ROUND_THRESHOLD)
    #     parallel_task_with_timeout(func_obj=SVGParser.extract_svg_lines,
    #                                pdf_file=pdf_file,
    #                                page_num=page_num)

    json_payload = {"lines_data": lines_data}

    if return_svg_size:
        json_payload.update({"svg_width": width,
                             "svg_height": height})
    if return_attributes:
        json_payload["lines_attributes"] = attributes

    # ________________________________________________________________________________
    # --- RETURN RESPONSE -> JSON WITH lines DATA ---
...
# ________________________________________________________________________________
# --- EXTRACT LINES DATA - S3 ---
@svg_lines_router.post(path="/extract-lines-data-s3/",
                       responses={200: {}, 500: {}, 503: {}},
                       status_code=status.HTTP_200_OK,
                       response_class=UJSONResponse,
                       tags=["SVG Parsing - Extract LINES as JSON data", "S3"],
                       summary="Extract LINES data in JSON format.")
async def extract_lines_data_s3(files_data: SVGParsingFilesDataS3) -> Response:
    # ________________________________________________________________________________
    ...
    # --- EXTRACT LINES DATA ---
    # ________________________________________________________________________________
    lines_data, width, height, attributes = SVGParser.extract_svg_lines(pdf_file_obj=pdf_file_bytes,
