...
# ________________________________________________________________________________
# --- FastAPI ROUTER ---
svg_circles_router = APIRouter(prefix="/v1/svg-parsing")


# ________________________________________________________________________________
# --- EXTRACT CIRCLES DATA ---
@svg_circles_router.post(path="/extract-circles-data/",
                         responses={200: {}, 500: {}, 503: {}},
                         status_code=status.HTTP_200_OK,
                         response_class=UJSONResponse,
                         tags=["SVG Parsing - Extract CIRCLES as JSON data"],
                         summary="Extract CIRCLES data in JSON format.")
async def extract_circles_data(return_svg_size: bool = True,
    ...
    # --- EXTRACT circles DATA ---
    circles_data, width, height, attributes = SVGParser.extract_svg_circles(pdf_file_obj=pdf_file,
                                                                            page_num=page_num,
                                                                            round_threshold=settings.ROUND_THRESHOLD)
    # \
    #     parallel_task_with_timeout(func_obj=SVGParser.extract_svg_circles,
    #                                pdf_file=pdf_file,
    #                                page_num=page_num)

    json_payload = {"circles_data": circles_data}
...
# ________________________________________________________________________________
# --- EXTRACT CIRCLES DATA - S3 ---
@svg_circles_router.post(path="/extract-circles-data-s3/",
                         responses={200: {}, 500: {}, 503: {}},
                         status_code=status.HTTP_200_OK,
                         response_class=UJSONResponse,
                         tags=["SVG Parsing - Extract CIRCLES as JSON data", "S3"],
                         summary="Extract CIRCLES data in JSON format.")
async def extract_circles_data_s3(files_data: SVGParsingFilesDataS3) -> Response:
    ...
    # --- EXTRACT CIRCLES DATA ---
    # ________________________________________________________________________________
    circles_data, width, height, attributes = (SVGParser.
                                               extract_svg_circles(pdf_file_obj=pdf_file_bytes,
